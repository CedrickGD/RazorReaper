...
public class LicenseService : ILicenseService
{
    ...
    public string CurrentLicenseKey => _preferences.Get(LicenseKeyPref, string.Empty);
    ...
    public LicenseService(HttpClient httpClient, IHwidService hwidService, IPreferencesStore preferences)
    {
        ...
        {
            var cachedExpiry = _preferences.Get(ExpiresAtPref, string.Empty);
            var expiresAt = string.IsNullOrWhiteSpace(cachedExpiry) ? null : cachedExpiry;
            if (!IsExpired(expiresAt))
            {
                IsActivated = true;
                ExpiresAt = expiresAt;
                LicenseType = _preferences.Get(LicenseTypePref, string.Empty) is { Length: > 0 } t ? t : null;
            }
    ...
    {
        _preferences.Set(ExpiresAtPref, ExpiresAt ?? string.Empty);
        _preferences.Set(LicenseTypePref, LicenseType ?? string.Empty);
    }
    ...
    private void ClearCachedState()
    {
        ...
        _preferences.Remove(LicenseKeyPref);
        _preferences.Remove(ExpiresAtPref);
        _preferences.Remove(LicenseTypePref);
    }

    /// <summary>
    /// Whether a non-2xx validate response is a verdict on the key itself. The server rejects a
    /// key with 400/403/404 (and a 2xx <c>ok:false</c>); 401 is the install-signature gate
    /// ("Invalid install signature." — unregistered install, clock skew), 408/429 and 5xx are
    /// backend trouble. None of those say anything about the license, so the cached state stays.
    /// </summary>
    internal static bool IsExplicitLicenseRejection(HttpStatusCode status)
    {
        var code = (int)status;
    ...
    public async Task<(bool Success, string Message)> ActivateLicenseAsync(string licenseKey)
    {
        try
        {
            ...
            {
                _preferences.Set(LicenseKeyPref, licenseKey);
                IsActivated = true;
    ...
    public async Task<(bool Success, string Message)> ValidateLicenseAsync()
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode && !IsExplicitLicenseRejection(response.StatusCode))
            {
                // 401 (install signature not accepted yet / clock skew), 429 or 5xx: the server
                // never looked at the key. Treat it like an unreachable server — keep the
                // last-known state and keep polling. Wiping the key here would de-activate a
                // paying user every time registration lags or the clock is off.
                return (false, result?.Error ?? "License server unavailable. Keeping the last verified state.");
            }
